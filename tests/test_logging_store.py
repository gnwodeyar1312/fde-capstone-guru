...
class TestSummaryStats:
    ...
    def test_counts_by_final_action(self, db_path):
        for i in range(3):
            log_decision(
                make_decision_record(ticket_id=f"T-{i}", final_action="sent"), db_path
            )
        for i in range(2):
            log_decision(
                make_decision_record(ticket_id=f"E-{i}", final_action="escalated"),
                db_path,
            )
        stats = get_summary_stats(db_path)
    ...
    def test_average_confidence(self, db_path):
        log_decision(
            make_decision_record(ticket_id="T-1", intent_confidence=0.90), db_path
        )
        log_decision(
            make_decision_record(ticket_id="T-2", intent_confidence=0.80), db_path
        )
        stats = get_summary_stats(db_path)
